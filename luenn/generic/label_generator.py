...
def label_generator(tar_em, frame_max, z_range, slide, box_size):
	...
	for f in frame_ids:
		...
		for n in range(len(xyz)):
			...
			if 2 <= xi < 254 - box_size and 2 <= yi < 254 - box_size:
				...
				channel_0 = dist_unit * np.cos(zr)*1000.0
				channel_1 = dist_unit * np.sin(zr)*1000.0
				left_side = box_size // 2
				right_side = left_side + 1
				y_train[f, 0, xi - left_side:xi + right_side, yi - left_side:yi + right_side] += channel_0
				y_train[f, 1, xi - left_side:xi + right_side, yi - left_side:yi + right_side] += channel_1
...
def label_dist(xm, ym, box_size):
	mu = [2 + ym, 2 + xm]
	sigma = [1.0, 1.0]
	xs = np.arange(0, box_size)
	ys = np.arange(0, box_size)
