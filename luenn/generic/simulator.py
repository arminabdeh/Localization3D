...
def frame_simulation(GTs, simulator, img_size):
    """
    Generates simulated frames from ground truth emitters (GTs) using the provided simulator.

    Parameters:
    - GTs: Ground truth emitters, an instance of decode.EmitterSet.
    - simulator: The simulation engine used to generate frames.
    - img_size: The size of the output images.

    Returns:
    - x_sim_lue: Tensor of simulated frames.
    """
    n_frames = GTs.frame_ix.max() + 1
    x_sim = np.zeros((n_frames, 1, img_size[0], img_size[1]))

    for f in range(n_frames):
        GTs_Frame = GTs[GTs.frame_ix == f]
        xyzs = GTs_Frame.xyz
        intensity = GTs_Frame.phot
        frame = simulator.psf.forward(xyzs, intensity)
        frame_bg, bg = simulator.background.forward(frame)
        frame_bg_n = simulator.noise.forward(frame_bg)
        x_sim[f, 0, :, :] = frame_bg_n[0, :, :].cpu().numpy().T

    x_sim = torch.tensor(x_sim).float()
    return x_sim
...
class fly_simulator:
    ...
    def generate_engine(self, tar_em, mode='train'):
        ...
        if self.report:
            ...
            print('-' * 50)
        y_sim = label_generator(tar_em, f_max, self.z_range,
...
class validation_simulator:
    ...
    def sample(self):
        ...
        if self.report:
            ...
            print()
        if self.with_label:
            y_sim = label_generator(tar_em, total_frames, self.z_range, self.slide, self.box_size)
        else:
            y_sim = None
